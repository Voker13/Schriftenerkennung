...
public class Picture {
    ...
    private static int getCharacterHeight2(RGB[][] pixel,int h, int xStart, int xEnd, int yStart) {
   	int y = 0;
   	for (int i = xStart; i < xEnd; i++) {
   	    for (int j = yStart; j < h; j++) {
   		if (pixel[i][j].isBlack() && j > y) {
   		    y = j;
   		}
   	    }
   	}
     ...
     *            = RGB[][]
     * @param w
     *            die breite in der gesucht werden soll (von 0 bis w)
     * @param y
     *            die reihe in pixel
     * @return
     */
	...
	System.out.println("Search character height for: height:" + bi.getHeight() + ", minX:" + minX + ", maxX:" + (minX+width) + ", minY:" + minY);

	int height = getCharacterHeight2(pixel, bi.getHeight(),minX,(minX+width), minY);
